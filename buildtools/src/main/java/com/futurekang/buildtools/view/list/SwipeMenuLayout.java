package com.futurekang.buildtools.view.list;
...
/**
 * form GitHub : https://github.com/mcxtzhang/SwipeDelMenuLayout
 * 【Item侧滑删除菜单】
...
public class SwipeMenuLayout extends ViewGroup {
    ...
    //存储的是当前正在展开的View
    private static SparseArray<SwipeMenuLayout> mViewCache;
    ...
    public SwipeMenuLayout getViewCache() {
        for (int i = 0; i < mViewCache.size(); i++) {
            SwipeMenuLayout layout = mViewCache.get(mViewCache.keyAt(i));
            if (this == layout) {
                return layout;
            }
        }
        ...
        if (isSwipeEnable) {
            ...
            switch (ev.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    ...
                    //避免多个手指同时触摸，这里判断pointId只让第一个手指触摸生效
                    if (ev.getPointerId(ev.getActionIndex()) != 0) {//如果有别的指头摸过了，那么就return false。这样后续的move..等事件也不会再来找这个View了。
                        return false;
                    }
...
//                        }
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    float gap = (mLastP.x - ev.getRawX()) * MOVE_FACTOR;
    ...
    public void smoothExpand() {
        ...
        invalidate();*/
        //展开就加入ViewCache：
        mViewCache.put(this.hashCode(), SwipeMenuLayout.this);
...
//        mViewCache = null;
        ...
        //设置先减速后加速的插值器
        mCloseAnim.setInterpolator(new AnticipateOvershootInterpolator());
        ...
        mCloseAnim.setDuration(500).start();
...
/*    @Override
     ...
     * 在RecyclerView里，视情况而定
     */
    public void quickClose() {
        if (this == getViewCache()) {
            //先取消展开动画
            cancelAnim();
            getViewCache().scrollTo(0, 0);//关闭
            closeViewCache();
        }
    }

    //createBy futurekang 2019-6-28 17:03:00
    //设置存放内容的子View的长按事件,用以代替itemView的长按事件
    private void setContentViewListener() {
        if (getChildCount() > 0) {
            this.getChildAt(0).setOnLongClickListener(new OnLongClickListener() {
                @Override
                public boolean onLongClick(View v) {
                    if (contentLongClickLitener != null) {
                        contentLongClickLitener.onLongClick(v);
                    }
                    return false;
                }
            });
            this.getChildAt(0).setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    if (contentClickLitener != null) {
                        contentClickLitener.onClick(v);
                    }
                }
            });
        }
    ...
    OnContentLongClickLitener contentLongClickLitener;
    OnContentClickLitener contentClickLitener;

    public interface OnContentLongClickLitener {
        void onLongClick(View v);
    }

    public interface OnContentClickLitener {
        void onClick(View v);
    }

    public void setContentLongClickLitener(OnContentLongClickLitener contentLongClickLitener) {
        this.contentLongClickLitener = contentLongClickLitener;
    }

    public OnContentLongClickLitener getContentLongClickLitener() {
        return contentLongClickLitener;
    }

    public void setContentClickLitener(OnContentClickLitener contentClickLitener) {
        this.contentClickLitener = contentClickLitener;
    }
}
