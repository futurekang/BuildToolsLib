...
public class SpringScrollView extends NestedScrollView {
    ...
    //判定上拉下拉操作的最短距离
    private static final float MOVE_MINI_HEIGHT = 800;
    //最大距离
    private static final float MOVE_MAX_HEIGHT = 1000;
    ...
    private boolean isDownStatus = false;
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                canPullUp = false;
                isMoved = false;
                Log.d(TAG, "onGlobalLayout: " + contentView.getHeight());
                break;
            case MotionEvent.ACTION_MOVE:
                ...
                if (shouldMove && deltaY < MOVE_MAX_HEIGHT) {
                    //计算偏移量
                    ...
                    if (actionListener != null) {
                        if (isDownStatus && deltaY > 0 && deltaY > MOVE_MINI_HEIGHT) {
                            actionListener.OnPullDown();
                            isDownStatus = false;
                        } else if (isUpStatus && deltaY < 0 && deltaY < -MOVE_MINI_HEIGHT) {
                            actionListener.OnPullUp();
                            isUpStatus = false;
                        }
                    }
                }
